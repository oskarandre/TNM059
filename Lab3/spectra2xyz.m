...
%
k= 100/sum(illumination.*xyz(:,2));%calculate the normalization factor
X= k * sum(reflectance.*illumination.*xyz(:,1)); % this gives the X-tristimulus value
Y= k * sum(reflectance.*illumination.*xyz(:,2)); % this gives the Y-tristimulus value
Z= k * sum(reflectance.*illumination.*xyz(:,3));% this gives the Z-tristimulus value
%
%% Test your code
